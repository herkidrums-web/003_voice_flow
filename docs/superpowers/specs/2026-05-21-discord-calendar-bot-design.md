...
| 트리거 조건 | (a) 이미지 첨부 OR (b) `"일정"`/`"캘린더"`로 시작 OR (c) 캘린더 명사("캘린더"/"일정") + 등록/수정 동사("등록"·"추가"·"삭제"·"수정" 등)가 문장 어디든 함께 등장 |
...
| 위험 | 대응 |
|---|---|
| 키워드 오트리거 ("오늘 일정 어때?") | 명사만 있고 등록/수정 동사가 없으면 `_handle_claude`로 흘림. 명사+동사 동시 등장 시에만 calendar |
