...
### `custom_terms.txt` 매핑 형식
- `오인식=>정답` 한 줄에 하나 (예: `강대국=>임대묵`) — DictionaryAgent가 transcript에서 `str.replace`로 일괄 치환
- 빈발 인명은 별도로 ALWAYS 섹션(파일 상단)에 정답 표기만 추가 → STT 프롬프트에 항상 주입되어 미래 녹취에서 오인식 자체를 줄임
- `#`로 시작하는 줄은 주석. `=>` 없는 평문 라인은 매핑이 아닌 사전 컨텍스트로만 사용됨

---

## ★★ 고유명사 수정 워크플로우 (자연어 트리거)

담당이 "고유명사 수정하자", "X = Y로 교정", "X 잘못 들렸어, 사실은 Y야" 등으로 요청하면 **5단계 자동 처리** (사람이 수동으로 매번 다 못 하니까 한 번 말로 지시).

### 트리거 예시
- "고유명사 수정하자 강대국 = 임대묵"
- "STT가 X를 Y로 잘못 들었다, 교정해줘"
- "이번 브리핑에 X는 Y로 바꿔"

### 자동 처리 5단계
1. **`custom_terms.txt` 매핑 추가** — 매핑 섹션(파일 후반부 `=>` 라인 모음)에 `오인식=>정답` 추가. 동일 매핑 있으면 skip
2. **ALWAYS 섹션 갱신** (선택) — 정답이 빈발 인명이면 파일 상단 `[ALWAYS]` 섹션에도 정답 표기 추가 (STT 프롬프트 주입용)
3. **Notion 전수 검색** — `notion-search`로 오인식 문자열이 포함된 페이지 전부 찾기. 인명이 아닌 일반 명사(예: "강대국=국가 의미")는 문맥 확인 후 인명 등장 페이지만 선별
4. **Notion 일괄 치환** — 각 페이지 `notion-update-page` 호출
   - 본문: `update_content` + `replace_all_matches=true`
   - 페이지 제목·요약 등 properties에 들어가있으면 `update_properties`로 별도 갱신 (개인기록_DB의 `회의 이름`, `요약` 필드)
5. **Wiki 일괄 치환** — `grep -rn "오인식" wiki/`로 등장 위치 모두 찾아 Edit 도구로 치환 (index.md 미팅노트 제목, 누적 인텔리전스 본문 등)

### 처리 보고
완료 후 담당에게 한 줄 보고:
- `custom_terms.txt`: 매핑 1건 추가
- Notion: N개 페이지 수정 (페이지 제목 K개)
- Wiki: M개 파일 수정

### 주의
- 인명이 일반 단어와 겹치는 경우 (예: "강대국" = 인명/국가) — Notion 검색 결과에서 문맥 확인 후 인명 등장 페이지만 수정. 불확실하면 담당에 확인
- 본인 표기 (`이성우 담당` 등)는 절대 변경 금지
- 임원 호칭 ("상무" 등) 회귀 발생 여부 재확인
...
수정/삭제도 "캘린더"/"일정" 키워드로 트리거 (예: "캘린더 방금 거 15시로", "캘린더 6/22 김태원 저녁 삭제") — `_handle_calendar`가 캘린더를 검색해 update/delete.

calendar 세션은 일반 대화 세션과 분리됨 — calendar 프롬프트의 "JSON만 응답" 지시가 일반 대화로 새지 않음. `/reset`은 두 세션 모두 초기화.
